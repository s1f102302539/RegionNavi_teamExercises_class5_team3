...
import FollowList from '@/app/components/pages/FollowList';
// ★ 追加
...
const componentMap: { [key: string]: React.ComponentType<any> } = {
  ...
  // ★ ここには MyPage や UserProfile は動的判定があるので登録しなくてもOK（登録しても良い）
};
...
const getComponent = (
  ...
  if (viewKey === 'mypage') {
    // ★ side を渡す
    if (currentUser) return <MyPage side={side} />;
    return timelineComponent;
  }
  
  if (viewKey === 'userprofile' && targetUserId) {
    // ★ UserProfileコンポーネントを使用し、sideとuserIdを渡す
    return <UserProfile userId={targetUserId} side={side} />;
  }
...
export default function HomePageController({ leftTimeline, rightTimeline }: HomePageControllerProps) {
  ...
  const followType = params.get('type'); 
